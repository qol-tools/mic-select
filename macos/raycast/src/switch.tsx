/** Switch Microphone command implementation. */
...
export default function SwitchMicrophone() {
  ...
  const handleSwitch = async (source: AudioSource) => {
    try {
      ...
      if ("error" in response) {
        ...
        // Pop the view after successful switch
        setTimeout(() => {
          // Raycast will automatically close the view
        }, 500);
      }
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
  ...
  return (
    <List
      isLoading={isLoading}
      searchBarPlaceholder="Search and switch microphone..."
